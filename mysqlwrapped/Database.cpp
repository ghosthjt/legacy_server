...
Database::OPENDB* Database::grabdb()
{
	if (!odb.get())									
	{
		...
		if (!mysql_real_connect(&odb -> mysql,		host.c_str(),	user.c_str(),	password.c_str(),
			...
			i_log_system::get_instance()->write_log(loglv_error, "connect to db failed, host = %s, database = %s, port = %d, err =\r\n %s",
...
void Database::keep_alive()
{
	if (odb.get()){
		...
		if (ping_result != 0){
			const char* err = mysql_error(&odb->mysql);
			if (strstr(err, "Commands out of sync;")) {
				i_log_system::get_instance()->write_log(loglv_error, "ping db err, result is not been freed, last sql = %s", odb->last_sql_.c_str());
			}
			else {
				i_log_system::get_instance()->write_log(loglv_error, "ping db err, ret = %d, %s", ping_result, err);
			}
			mysql_close(&odb->mysql);
			odb.reset();
		}
