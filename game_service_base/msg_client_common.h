...
enum
{
    ...
    GET_CLSID(msg_everyday_sign_req),
    GET_CLSID(msg_luck_wheel_req),
    GET_CLSID(msg_subsidy_req),
    ...
    GET_CLSID(msg_form_req) = 1200,
	msg_client_id_this_game_begin = 1300,
};
...
public:
	...
	int			read(boost::property_tree::ptree& data_s)
	{
		...
		read_jstring(pwd_old_, max_name, data_s);
		read_jstring(pwd_, max_name, data_s);
...
public:
	...
	int			read(boost::property_tree::ptree& data_s)
	{
		...
		read_jstring(acc_name_, max_name, data_s);
		read_jstring(pwd_, max_name, data_s);
		read_jstring(platform_id, max_name, data_s);
		return ERROR_SUCCESS_0;
	}
...
public:
	...
	int			read(boost::property_tree::ptree& data_s)
	{
		...
		read_jstring(acc_name_, max_name, data_s);
		read_jstring(pwd_, max_name, data_s);
		read_jstring(head_icon_, max_name, data_s);
		read_jvalue(relate_iid_, data_s);
		return ERROR_SUCCESS_0;
	}
...
public:
	...
	{
		msg_from_client::read(data_s);
		read_jstring(uid_, max_name, data_s);
		read_jstring(uname_, max_name, data_s);
		read_jvalue(vip_lv_, data_s);
		read_jstring(head_icon_, max_name, data_s);
		read_jstring(platform_, max_name, data_s);
		read_jstring(sn_, max_name, data_s);
		read_jstring(url_sign_, max_name, data_s);
...
public:
	...
	{
		msg_from_client::read(data_s);
		read_jstring(FromID, max_guid, data_s);
		read_jstring(uidx_, max_guid, data_s);
		read_jstring(uid_, max_guid, data_s);
		read_jstring(pwd, max_guid, data_s);
		read_jstring(uname_, max_guid, data_s);
		read_jstring(head_pic_, max_guid, data_s);
		read_jstring(session_, max_guid, data_s);
		return ERROR_SUCCESS_0;
	}
...
public:
	...
	{
		msg_from_client::read(data_s);
		read_jvalue(page_, data_s);
		read_jvalue(page_set_, data_s);
		return ERROR_SUCCESS_0;
	}
...
public:
	...
	int			read(boost::property_tree::ptree& data_s)
	{
		...
		read_jstring(game_ins_, max_guid, data_s);
		return ERROR_SUCCESS_0;
	}
...
class msg_everyday_sign_req : public msg_authrized<remote_socket_ptr>
{
public:
    int			type_;		//0查询签到情况 1查询奖励列表 2签到
    msg_everyday_sign_req()
    {
        head_.cmd_ = GET_CLSID(msg_everyday_sign_req);
    }
...
class msg_luck_wheel_req : public msg_authrized<remote_socket_ptr>
{
public:
    int			type_;		//0请求剩余抽奖次数 1请求全局中奖信息 2使用次数抽奖 3使用钻石抽奖
    msg_luck_wheel_req()
    {
        head_.cmd_ = GET_CLSID(msg_luck_wheel_req);
    }
...
class msg_subsidy_req : public msg_authrized<remote_socket_ptr>
{
public:
    int			type_;		//0查询当天破产补助情况 1领取破产补助
    msg_subsidy_req()
    {
        head_.cmd_ = GET_CLSID(msg_luck_wheel_req);
    }
...
public:
	...
	int			read(boost::property_tree::ptree& data_s)
	{
		...
		read_jvalue(type_, data_s);
		read_jvalue(page_, data_s);
		read_jvalue(page_count_, data_s);
		return ERROR_SUCCESS_0;
	}
...
public:
	...
	{
		msg_authrized::read(data_s);
		read_jstring(to_name_, max_guid, data_s);
		read_jvalue(channel_, data_s);
		read_jstring(content_, 256, data_s);
		return ERROR_SUCCESS_0;
	}
...
class msg_ping_req : public msg_base<remote_socket_ptr>
{
public:
...
public:
	...
	{
		msg_from_client::read(data_s);
		read_jstring(device_id, max_name, data_s);
		return ERROR_SUCCESS_0;
	}
...
public:
	...
	int			read(boost::property_tree::ptree& data_s)
	{
		...
		read_jvalue(match_id_, data_s);
		read_jstring(game_ins_, max_guid, data_s);
		read_jvalue(score_, data_s);
		read_jvalue(time_left_, data_s);
		return ERROR_SUCCESS_0;
	}
...
public:
	...
	int			read(const char*& data_s, unsigned int l)
	{
		...
		read_data(isagent_, data_s);
		read_data(isinit_, data_s);
		return 0;
	}
};
...
public:
	...
	int			read(boost::property_tree::ptree& data_s)
	{
		...
		read_jstring(email_, max_guid, data_s);
...
public:
	...
	int			read(boost::property_tree::ptree& data_s)
	{
		msg_player_info_ex::read(data_s);
...
public:
	...
	{
		msg_base::read(data_s);
		read_jstring(sn_, max_guid, data_s);
...
public:
	...
	{
		msg_authrized::read(data_s);
		read_jstring(platform_id_, max_guid, data_s);
		return ERROR_SUCCESS_0;
	}
...
class		msg_koko_user_data_change : public msg_base<boost::shared_ptr<world_socket>>
{
public:
	std::string	uid_;
	int			itemid_;
