...
void telfee_match_info::update()
{
	...
	if (state_ == state_wait_start_ || state_ == state_wait_balance){
		...
		if ((pt_now - pt_last_refresh_).total_seconds() > 1){
			...
			for (int i = (int)vscores_.size() - 1 ; i >= 0 ; i--)
			{
				...
				logic_ptr tpl = pp->the_game_.lock();
				//如果玩家已被淘汰
				if (!tpl.get() || tpl->is_match_game_ != iid_){
					msg_int.operator_ = 2;
				}
				else{
					msg_int.operator_ = 0;
				...
				//如果玩家被淘汰,移除
				if (msg_int.operator_ == 2){
					vscores_.erase(vscores_.begin() + i);
				}
			}
			pt_last_refresh_ = boost::posix_time::microsec_clock::local_time();
		}
...
std::vector<logic_ptr> telfee_match_info::distribute_player_to_match()
{
	...
	while (itn != registers_.end())
	{
		...
		if (itn->second == 0){
			...
			if (!logic.get()){
				logic.reset(new logic_ptr::element_type(iid_ == 0 ? 1 : iid_));
