...
class mem_connector : public native_socket
{
public:
	mem_connector(boost::asio::io_service& ios);
...
public:
	...
	int				reconnect(int switch_connection = 0)
	{
		...
		if (ip_ != ""){
			...
			if (r == 0) {
				msg_cache_updator_register<mem_connector_ptr > msg;
				stream_buffer sd = build_send_stream(&msg);
				if (async_s_->send_data(sd) != 0) { return -2; }
			}
			return r;
		}
		return 0;
	...
	void		keep_alive()
	{
		...
		int r = reconnect(0);
		if (r != 0)	{
			glb_http_log.write_log("!!!!!!all cache is down!!!!!");
		}
	}
