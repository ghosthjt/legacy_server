...
mem_cache_client::mem_cache_client(int gameid) : timer_(ios_)
{
	game_id_ = gameid;
	sn_gen_ = 1;
}

int mem_cache_client::reconnect(int switch_connection /*= 0*/)
{
...
void mem_cache_client::keep_alive()
{
	boost::system::error_code ec;
	if (!s_.get()) return; 
	int r = reconnect(0);
...
boost::shared_ptr<msg_base<mem_connector_ptr>> mem_cache_client::create_msg(unsigned short cmd)
{
...
int mem_cache_client::send_cache_cmd(std::string key, std::string cmd, val_t val, ret_t& ret)
{
...
int mem_cache_client::async_send_cache_cmd(std::string key, std::string cmd, val_t val, int* out_sn /*= nullptr*/)
{
	return do_send_cache_cmd(key, cmd, val, nullptr, 1000, out_sn);
}

void mem_cache_client::close()
{
	boost::system::error_code ec;
	timer_.cancel(ec);
	ios_.stop();
...
int mem_cache_client::get_cache_result(std::vector<msg_ptr>& msg_lst, mem_connector_ptr ref_socket, unsigned int time_out /*= 1000*/)
{
	pickup_msg(ref_socket, msg_lst);
	return 0;
}

int mem_cache_client::get_cache_result(msg_cache_cmd_ret<mem_connector_ptr>& msg_ret, mem_connector_ptr ref_socket, std::string sequence, unsigned int time_out /*= 1000*/)
{
	int ret = -3;
...
void mem_cache_client::pickup_msg(mem_connector_ptr ref_socket, std::vector<msg_ptr>& msg_lst)
{
	bool got_msg = false;

	do {
		got_msg = false;
		auto msg = pickup_msg_from_socket(ref_socket, mem_cache_client::create_msg, got_msg);
		if (msg.get()){
...
void mem_cache_client::send_notify(boost::system::error_code ec, unsigned int bytes_transferred)
{
	if (ec.value() == 0) {
...
void mem_cache_client::pickup_value(msg_cache_cmd_ret<mem_connector_ptr>&msg, std::string& ret)
{
...
void mem_cache_client::pickup_value(msg_cache_cmd_ret<mem_connector_ptr>&msg, longlong& ret)
{
	if (msg.return_code_ == 0) {
		ret = msg.value_;
	}
...
void mem_cache_client::set_value(msg_cache_cmd<mem_connector_ptr>&msg, std::string val)
{
	COPY_STR(msg.str_value_, val.c_str());
	msg.val_type_ = 1;
}

void mem_cache_client::set_value(msg_cache_cmd<mem_connector_ptr>&msg, longlong val)
{
	msg.value_ = val;
	msg.val_type_ = 0;
}
...
template<class val_t>
int mem_cache_client::do_send_cache_cmd(std::string key, std::string cmd, val_t val, msg_cache_cmd_ret<mem_connector_ptr>* msg_ret, int time_out /*= 1000*/, int* out_seq /*= nullptr*/)
{
	boost::system::error_code ec;
