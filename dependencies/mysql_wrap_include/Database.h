...
#define BEGIN_UPDATE_TABLE(table)\
	...
	str_field = "update `" + lx2s(table) + "` set "	

#define BEGIN_INSERT_TABLE(table)\
	std::string str_field;\
	str_field = "insert into `" + lx2s(table) + "` set "

#define BEGIN_REPLACE_TABLE(table)\
	std::string str_field;\
	str_field = "replace into `" + lx2s(table) + "` set "

#define BEGIN_DELETE_TABLE(table)\
	std::string str_field;\
	bool bimmediate = false;\
	str_field = "delete from `" + lx2s(table) + "`"

#define SET_FIELD_VALUE(f, v)\
	str_field += "`" + lx2s(f) + "` = '" + lx2s(v) + "',"

//value不用''包含
#define SET_FIELD_VALUE_NO_COMMA(f, v)\
	str_field += "`" + lx2s(f) + "` = " + lx2s(v) + ","

#define ADD_FIELD_VALUE(f, v)\
	str_field += "`"+ lx2s(f) + "` = " + "`" + lx2s(f) + "` + " + lx2s(v) + ","

#define SET_FINAL_VALUE(f, v)\
	str_field += "`"+ lx2s(f) + "` = '" + lx2s(v) + "'"

//value不用''包含
#define SET_FINAL_VALUE_NO_COMMA(f, v)\
	str_field += "`"+ lx2s(f) + "` = " + lx2s(v)
